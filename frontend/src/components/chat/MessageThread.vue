<template>
  <div class="message-thread d-flex flex-column flex-grow-1" style="height: 100%;">
    ...
    <template v-else>
      ...
      <div v-if="pendingImage" class="pending-attachment pa-2 d-flex align-center" style="gap: 10px;">
        <img :src="pendingImagePreview" alt="preview" style="width: 56px; height: 56px; object-fit: cover; border-radius: 8px;" />
        <div class="flex-grow-1">
          <div class="text-body-2 font-weight-medium">{{ pendingImage.name }}</div>
          <div class="text-caption" style="color: #888;">{{ formatFileSize(pendingImage.size) }}</div>
        </div>
        <v-btn icon size="x-small" variant="text" @click="clearPendingImage">
          <v-icon size="18">mdi-close</v-icon>
        </v-btn>
        <v-btn color="primary" size="small" :loading="sending" @click="confirmSendImage">
          Gửi ảnh
        </v-btn>
      </div>

      <!-- File preview before sending -->
      <div v-if="pendingFile" class="pending-attachment pa-2 d-flex align-center" style="gap: 10px;">
        ...
        <div class="flex-grow-1">
          ...
          <div class="text-caption" style="color: #888;">{{ formatFileSize(pendingFile.size) }}</div>
        </div>
        <v-btn icon size="x-small" variant="text" @click="clearPendingFile">
          <v-icon size="18">mdi-close</v-icon>
        </v-btn>
        <v-btn color="primary" size="small" :loading="sending" @click="confirmSendFile">
          Gửi file
        </v-btn>
      </div>

      <!-- Input area — Figma style: attachment icon, text field, send button -->
      <div class="pa-3 d-flex align-center chat-input-area" style="gap: 8px;">
        <!-- Sticker picker -->
        <v-btn icon variant="text" size="small" color="grey" :disabled="sending" @click="openStickerPicker" title="Gửi sticker">
          <v-icon>mdi-sticker-emoji</v-icon>
        </v-btn>
        <!-- Image upload -->
        <v-btn icon variant="text" size="small" color="grey" :disabled="sending" @click="triggerImageUpload" title="Gửi ảnh">
          <v-icon>mdi-image-outline</v-icon>
        </v-btn>
        <!-- File upload -->
        <v-btn icon variant="text" size="small" color="grey" :disabled="sending" @click="triggerFileUpload" title="Gửi file/tài liệu">
        ...
        <v-btn
          variant="text"
          color="primary"
          :loading="sending && !pendingImage && !pendingFile"
...
const emit = defineEmits<{
  send: [content: string];
  'send-sticker': [sticker: StickerDetail];
...
function onFileSelected(e: Event) {
  ...
  if (file.size > 50 * 1024 * 1024) {
    syncSnack.value = { show: true, text: 'File quá lớn (tối đa 50 MB)', color: 'error' };
    target.value = '';
    return;
  }
...
function confirmSendImage() {
  ...
  emit('send-image', pendingImage.value);
  clearPendingImage();
}

function confirmSendFile() {
  if (!pendingFile.value) return;
  emit('send-file', pendingFile.value);
  clearPendingFile();
}
